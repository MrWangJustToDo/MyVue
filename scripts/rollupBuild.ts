...
const build = async (
  ...
  console.log(
    `[build] start build package ${packageName} with ${mode} mode ${isUMD ? "in umd format" : ""}`
  );
  try {
...
const rollupBuild = async (packageName: packages) => {
  const { allOtherDev, allOtherProd, allSingleOther, allSingleUMD, allUMDDev, allUMDProd } =
    await getRollupConfig(packageName);
